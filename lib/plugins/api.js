import Vue from 'vue';
import ApiHandler from '<% options.controllers %>';
...
function injectParamsIntoPath(path, params) {
    let assembledPath = path;
    if (params) {
        Object.keys(params).forEach(function (key) {
            assembledPath = assembledPath.replace(new RegExp(`/:${key}/g`), params[key]);
        });
    }
    ...
    Object.keys(controllerMapping).forEach(function (key) {
        ...
        if (context && context.path && context.verb) {
            api[key] = function (params, values) {
                return ApiHandler(injectParamsIntoPath(context.path, params), context.verb, values);
            }
